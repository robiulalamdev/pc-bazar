      ...
      <nav ref={navberRef} className="uppercase">
        <div className="relative cursor-pointer flex justify-between items-center gap-6 lg:gap-10 h-14 px-4 md:px-8 max-w-[1440px] mx-auto">
          <div className="flex-grow">
            <Link href="/">
              <Image className="w-32" src={logo} alt="navberImage" />
            </Link>
          </div>
          <div className="hidden lg:block">
            <Link
              href="/"
              className="text-black hover:text-blue-600 duration-100 font-semibold text-[13px]"
            >
              Home
            </Link>
          </div>
          ...
          <div className="hidden lg:block">
            <Link
              href="/pc-builder"
              className="text-black hover:text-primary duration-100 font-semibold text-[13px] uppercase"
            >
              PC Builder
            </Link>
          </div>
          ...
          {data?.user?.email ? (
            <div className="flex items-center gap-2">
              <button
                onClick={() => signOut()}
                className="w-24 h-8 bg-rose-600 hover:bg-rose-700 duration-300 flex justify-center items-center rounded"
              >
            ...
            <div className="hidden lg:block lg:flex justify-between items-center gap-6">
              <Link
                href="/signin"
                className="w-24 h-8 bg-blue-600 hover:bg-blue-700 duration-300 flex justify-center items-center rounded"
              >
        ...
        <div
          className={`bg-[#f1f5f9] absolute z-50 duration-300 border-r mt-[17px] lg:hidden flex flex-col items-start w-72 min-h-screen bg-white px-4 py-4
          ${open ? "left-0" : "-left-[300px]"}`}
        >
          <div className="lg:hidden w-full flex items-center px-2 text-left hover:bg-[#0029FF]">
            <Link
              href="/"
              className="text-black font-semibold hover:text-blue-600 w-full py-2"
            >
              Home
            </Link>
          </div>
